...
export function getInsightSortDate(insight: CollectionEntry<'insight'>) {
    return siteConfig.sortPostsByUpdatedDate &&
        insight.data.updatedDate !== undefined
...
export function sortMDByDate(insights: CollectionEntry<'insight'>[]) {
    return insights.sort((b, a) => {
        const aDate = getInsightSortDate(a).valueOf();
        const bDate = getInsightSortDate(b).valueOf();
        return aDate - bDate;
    });
}
...
export function groupInsightsByYear(insights: CollectionEntry<'insight'>[]) {
    return insights.reduce<Record<string, CollectionEntry<'insight'>[]>>(
        (acc, insight) => {
            const year = getInsightSortDate(insight).getFullYear();
            if (!acc[year]) {
                acc[year] = [];
            }
...
export function getUniqueTagsWithCount(
    insights: CollectionEntry<'insight'>[],
): [string, number][] {
    return [
